...
{
    /**
     * Two more client-mockup gaps: a "Designation" field alongside
     * Occupation/Company in Education & Career, and a first real set of
     * Lifestyle fields (previously the Lifestyle tab only showed hardcoded
     * "Not specified" placeholders — see member/profile.blade.php's comment
     * about this platform "not collecting most lifestyle details yet").
     * Kept as plain strings with fixed option lists in the edit forms
     * (same pattern as PartnerPreference.with_children), not a new
     * masterdata type — these aren't reference data anyone else needs to
     * manage, just a handful of fixed choices each.
     */
    public function up(): void
    {
        Schema::table('users', function (Blueprint $table) {
            $table->string('designation', 100)->nullable()->after('profession');
            $table->string('prayer', 30)->nullable()->after('family_values');
            $table->string('smoking', 20)->nullable()->after('prayer');
            $table->string('diet', 30)->nullable()->after('smoking');
            $table->string('exercise', 20)->nullable()->after('diet');
            $table->string('hobbies', 255)->nullable()->after('exercise');
            $table->string('living_arrangement', 30)->nullable()->after('hobbies');
        });

        // The `profile` VIEW (see add_family_background_fields_to_users_table for
        // why) needs these 7 added too, same as every other users column added
        // since that migration — otherwise anything querying the view directly
        // via Eloquent won't see them, even though the raw-SQL Profile::profiles()
        // (which uses `u.*`) picks them up automatically.
        ...
        DB::statement("
            ...
            SELECT
                ...
                `u`.`prayer` AS `prayer`, `u`.`smoking` AS `smoking`, `u`.`diet` AS `diet`,
                `u`.`exercise` AS `exercise`, `u`.`hobbies` AS `hobbies`,
                `u`.`living_arrangement` AS `living_arrangement`,
    ...
    {
        Schema::table('users', function (Blueprint $table) {
            $table->dropColumn(['designation', 'prayer', 'smoking', 'diet', 'exercise', 'hobbies', 'living_arrangement']);
        });

        // Restore the view to exactly what add_family_background_fields_to_users_table left it as.
        DB::statement('DROP VIEW IF EXISTS `profile`');
        DB::statement("
            CREATE VIEW `profile` AS
            SELECT
